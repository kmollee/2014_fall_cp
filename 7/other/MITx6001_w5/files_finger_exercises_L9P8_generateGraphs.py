...
def loadWords():
    ...
    fin = open('../ProblemSet4/words.txt', 'r')
    wordList = []
    for line in fin:
...
def doHashing(hashFunction):
    ...
    for w in wordList:
        ...
        # Create a frequency distribution of all the integers returned by the
        # hash function.
...
def plotHash(hashFunction):
    '''
    Takes as argument one of the hash functions defined.
    Plots frequency distribution of all the integers returned
    by the hash function.
    '''

    plt.bar(doHashing(hashFunction).keys(), doHashing(
        hashFunction).values(), facecolor='#9999ff', edgecolor='white')
...
'''
# use
plotHash(firstHashFunction)
#plotHash(secondHashFunction)
#plotHash(thirdHashFunction)
'''
